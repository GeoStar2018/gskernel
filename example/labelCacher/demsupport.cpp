...
DEMSupport::DEMSupport(const char* dem)
{
	...
	if (GsStringHelp::IsNullOrEmpty(dem))
	{
		GS_W << "dem dataset path is null";
		return;
	}
...
bool DEMSupport::HasDEM()
{
	return m_ptrDEMTile ? true : false;
}
bool DEMSupport::AssignDEMValue(int nLevel, KERNEL_NAME::GsRawPoint3D& pt)
{
	GsQuadKey key(nLevel, 0, 0);
	m_ptrPyramid->TileIndex(nLevel, pt.X, pt.Y, &key.Row, &key.Col);
	DEMTile* tile = EnsureTile(key);
	if (!tile)
		return false;
	pt.Z = tile->DEMValue(pt.X, pt.Y,pt.Z);
	return true;
}
//准备地形瓦片
